<?php
/**
 * msrevents functions and definitions
 *
 * @package msrevents
 */
...
require_once get_template_directory() . '/inc/msr-events-content.php';
require_once get_template_directory() . '/inc/msr-events-filter-bar.php';
require_once get_template_directory() . '/inc/msr-events-ecosystem.php';
require_once get_template_directory() . '/inc/msr-events-programme.php';
require_once get_template_directory() . '/inc/msr-events-admin.php';
require_once get_template_directory() . '/inc/msr-events-acf.php';
require_once get_template_directory() . '/inc/msr-events-options.php';
require_once get_template_directory() . '/inc/msr-events-search.php';
require_once get_template_directory() . '/inc/msr-events-footer.php';
require_once get_template_directory() . '/inc/msr-events-seo.php';
require_once get_template_directory() . '/inc/msr-events-portfolio.php';
require_once get_template_directory() . '/inc/msr-events-perf.php';
require_once get_template_directory() . '/inc/msr-events-ux.php';
require_once get_template_directory() . '/inc/msr-events-archive.php';
require_once get_template_directory() . '/inc/msr-events-single.php';
require_once get_template_directory() . '/inc/msr-events-about.php';
require_once get_template_directory() . '/inc/msr-events-routes.php';
require_once get_template_directory() . '/inc/msr-events-sections.php';
require_once get_template_directory() . '/inc/msr-events-publications.php';
 ...
 * @return void
 */
function msrevents_render_rich_text( $content ) {
 ...
 * @return void
 */
function msrevents_render_cta_link( $link, $class = 'btn btn-primary' ) {
 ...
 */
function msrevents_acf_image_url( $value ) {
	if ( '' === $value || null === $value ) {
		return '';
	}
	if ( is_numeric( $value ) ) {
	...
	if ( is_array( $value ) ) {
		...
		if ( ! empty( $value['ID'] ) ) {
			return msrevents_acf_image_url( $value['ID'] );
		}
		return '';
	}
	if ( is_string( $value ) ) {
		...
		if ( ctype_digit( $trim ) ) {
			return msrevents_acf_image_url( (int) $trim );
		}
		$maybe = maybe_unserialize( $trim );
		if ( $maybe !== $trim && ( is_array( $maybe ) || is_numeric( $maybe ) ) ) {
			return msrevents_acf_image_url( $maybe );
		}
		return $trim;
	}
	return '';
 ...
 */
function msrevents_sanitize_fa_icon_class( $value ) {
 ...
 */
function msrevents_award_term_slug( $term ) {
 ...
 */
function msrevents_video_embed_allowed_html() {
	return array(
		'iframe' => array(
			'src'             => true,
			'width'           => true,
 ...
 * @return string
 */
function msrevents_kses_video_embed( $html ) {
	return wp_kses( (string) $html, msrevents_video_embed_allowed_html() );
}
 ...
 */
function msrevents_render_video_embed( $video ) {
	...
	if ( false !== stripos( $video, '<iframe' ) ) {
		if ( preg_match( '/src=["\']([^"\']+)["\']/', $video, $src_match ) ) {
			...
			if ( $oembed ) {
				return msrevents_kses_video_embed( $oembed );
			}
		}
		return msrevents_kses_video_embed( $video );
	}
	...
	if ( preg_match( '/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([a-zA-Z0-9_-]{11})/', $video, $matches ) ) {
		...
		if ( $embed ) {
			return msrevents_kses_video_embed( $embed );
		}
	}
 ...
 * @return WP_Term[]
 */
function msrevents_get_filter_terms( $taxonomy, $parent = 0 ) {
 ...
 */
function msrevents_filter_tabs_query_args( $post_type, $term_slug, $taxonomy, $base_args = array() ) {
	$args = array_merge(
		array(
...
function msrevents_setup() {
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'msrevents_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'msrevents_setup' );
...
function msrevents_loadmore_ajax_handler() {
...
add_action('wp_ajax_loadmore','msrevents_loadmore_ajax_handler');
add_action('wp_ajax_nopriv_loadmore','msrevents_loadmore_ajax_handler');
